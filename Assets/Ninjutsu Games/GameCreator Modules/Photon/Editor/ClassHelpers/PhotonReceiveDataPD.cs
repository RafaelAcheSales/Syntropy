        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (list == null)
            {
                ...
                list.drawElementCallback = (rect, index, isActive, isFocused) =>
                {
                    ...
                    /*PhotonCustomData.VariableData.VariableType target = (PhotonCustomData.VariableData.VariableType)prop.GetArrayElementAtIndex(index).FindPropertyRelative("target").enumValueIndex;
                    switch (target)
                    {
                        case PhotonCustomData.VariableData.VariableType.Number: 
                            EditorGUI.PropertyField(rect, spProperty.FindPropertyRelative("numberVariable"), new GUIContent($"Number {index}"));
                            break;
                        case PhotonCustomData.VariableData.VariableType.Bool: 
                            EditorGUI.PropertyField(rect, spProperty.FindPropertyRelative("boolVariable"), new GUIContent($"Bool {index}"));
                            break;
                        case PhotonCustomData.VariableData.VariableType.Color: 
                            EditorGUI.PropertyField(rect, spProperty.FindPropertyRelative("colorVariable"), new GUIContent($"Color {index}"));
                            break;
                        case PhotonCustomData.VariableData.VariableType.String: 
                            EditorGUI.PropertyField(rect, spProperty.FindPropertyRelative("stringVariable"), new GUIContent($"String {index}"));
                            break;
                        case PhotonCustomData.VariableData.VariableType.Vector2: 
                            EditorGUI.PropertyField(rect, spProperty.FindPropertyRelative("vector2Variable"), new GUIContent($"Vector2 {index}"));
                            break;
                        case PhotonCustomData.VariableData.VariableType.Vector3: 
                            EditorGUI.PropertyField(rect, spProperty.FindPropertyRelative("vector3Variable"), new GUIContent($"Vector3 {index}"));
                            break;
                    }*/
                    // EditorGUI.PropertyField(rect, spProperty.FindPropertyRelative("variable"), new GUIContent($"Variable {index}"));
