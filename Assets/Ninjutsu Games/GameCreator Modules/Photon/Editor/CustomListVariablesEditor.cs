...
namespace NJG.PUN
{
    ...
    public class CustomListVariablesEditor : ListVariablesEditor
    {
        ...
        private ListVariablesNetwork.VarRPC varRPC;
        private int index = -1;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!initialized)
            {
                ...
                if (network != null)
                {
                    ...
                    varRPC = network.listVars.Find(p => p.variables == listVars);
                    index = network.listVars.IndexOf(varRPC);
                }
                hasComponent = varRPC != null;
                syncVariables = varRPC != null && varRPC.syncVariables;
                useNetworkInstantiation = varRPC != null && varRPC.useNetworkInstantiation;
                initialized = true;
                //CheckTypes();
            }
            ...
            using (var group = new EditorGUILayout.FadeGroupScope(this.section.state.faded))
            {
                if (group.visible)
                {
                    ...
                    if (useNetworkInstantiation) syncVariables = false;

                    if (varRPC != null && !CanInstantiate())
                    {
            ...
            if (hasChanged)
            {
                if ((varRPC != null) && network)
                {
                    ...
                    network.listVars.Remove(varRPC);
                    if (network.listVars.Count == 0 && !useNetworkInstantiation)
                    {
                ...
                else
                {
                    ...
                    varRPC = new ListVariablesNetwork.VarRPC() { variables = listVars };
                    varRPC.useNetworkInstantiation = useNetworkInstantiation;
                    varRPC.syncVariables = syncVariables;
                    network.listVars.Add(varRPC);

                    Debug.LogFormat("Added ListVariablesNetwork: {0}", varRPC.variables.gameObject.name);
