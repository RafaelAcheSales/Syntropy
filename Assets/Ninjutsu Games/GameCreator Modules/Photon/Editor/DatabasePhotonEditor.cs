    ...
    public class DatabasePhotonEditor : IDatabaseEditor
    {
        ...
        private void PaintDebug()
        {
            ...
            if (PhotonNetwork.InRoom)
            {
                ...
                string total = string.Format("Total Messages: Out {0,4} | In {1,4} | Sum {2,4}", gls.TotalOutgoingMessageCount, gls.TotalIncomingMessageCount, gls.TotalMessageCount);
                ...
                string average = string.Format("Messages Per Sec: Out {0,4} | In {1,4}", averageOut, averageIn);
                string bytesSummary = string.Format("Usage: In {0:F2}kB/sec | Out {1:F2}kB/sec", perSecBytesCommand_In, perSecBytesCommand_Out);
            ...
            else
            {
                ...
                if (PhotonNetwork.InRoom)
                {
                    EditorGUILayout.LabelField(string.Format("Name: {0}", PhotonNetwork.CurrentRoom.Name));
                    EditorGUILayout.LabelField(string.Format("Visible: {0}", PhotonNetwork.CurrentRoom.IsVisible));
                    EditorGUILayout.LabelField(string.Format("Open: {0}", PhotonNetwork.CurrentRoom.IsOpen));
                    EditorGUILayout.LabelField(string.Format("Is Offline: {0}", PhotonNetwork.CurrentRoom.IsOffline));
                    EditorGUILayout.LabelField(string.Format("Max Players: {0}", 
                        PhotonNetwork.CurrentRoom.MaxPlayers == 0 ? "Unlimited" : PhotonNetwork.CurrentRoom.MaxPlayers.ToString()));
                    ...
                    {
                        EditorGUILayout.LabelField(string.Format("[{0}] {1} : {2}", index, p.Key, p.Value));
                        index++;
                    }
                    EditorGUI.indentLevel--;
                }
