...
namespace NJG.PUN
{
    ...
    public class ListVariablesNetwork : MonoBehaviourPun, IMatchmakingCallbacks, IInRoomCallbacks
    {
        ...
        public List<VarRPC> listVars = new List<VarRPC>();
            ...
            //if (photonView.IsMine || photonView.IsSceneView)
            {
                for (int i = 0, imax = listVars.Count; i < imax; i++)
                {
                    ...
                    for (int e = 0, emax = rpc.variables.references.Length; e < emax; e++)
                    {
                        ...
                        if (rpc.useNetworkInstantiation)
                        {
                            ...
                            {
                                Debug.LogWarningFormat(prefab, "GameObject {0} needs to have PhotonView in order to be used for Photon Instantiation", prefab.name);
                                continue;
                            }*/
                            if (pool != null && !pool.ResourceCache.ContainsKey(prefab.name)) pool.ResourceCache.Add(prefab.name, prefab);
                        }
                        else if(rpc.syncVariables)
                        {
